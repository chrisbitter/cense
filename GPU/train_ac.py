...
from NeuralNetwork import nnFactory
...
def train(id):
    ...
    config_file = path.join(path.dirname(path.realpath(__file__)), "config.json")

    # if not os.path.exists(gpu_config_file):
    #     raise OSError("Config file for the GPU is missing.")
    # load config for the gpu training script
    ...
    train_parameters = path.join(root_folder, config["training_parameters"])

    actor_file = path.join(root_folder, config["actor_file"])
    critic_file = path.join(root_folder, config["critic_file"])

    actor_target_file = path.join(root_folder, config["actor_target_file"])
    critic_target_file = path.join(root_folder, config["critic_target_file"])

    new_data_folder = path.join(root_folder, 'training_data/data/new_data')
    data_file = path.join(root_folder, 'training_data/data/data.h5')

    training_signal = path.join(root_folder, "training_signal_" + id)
    alive_signal = path.join(root_folder, "alive_signal_" + id)
    ...
    # create models
    actor = nnFactory.actor_network(STATE_DIMENSIONS)
    actor_target = nnFactory.actor_network(STATE_DIMENSIONS)
    critic = nnFactory.critic_network(STATE_DIMENSIONS)
    critic_target = nnFactory.critic_network(STATE_DIMENSIONS)
