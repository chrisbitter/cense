...
while True:
        ...
        if new_data_file.endswith('.h5'):
            ...
            with h5py.File(new_data_folder + new_data_file, 'r') as f:
                if states is None:
                    ...
                    suc_states = np.concatenate([suc_states, f['suc_states'][:]])
    ...
    if states is None or not states.shape[0]:
        ...
        for epoch in range(epochs):
            ...
            max_target_q_values = np.amax(target_q_values, axis=1) * discount_factor * np.invert(batch_terminals)
