...
class Interface():
    ...
    def __init__(self):
        ...
        mw.show()
        mw.setWindowTitle('CENSE Demonstrator')
        ...
        self.statusAction = QtWidgets.QAction(QtGui.QIcon(), 'Start', mw)
        self.statusAction.triggered.connect(self.changestatus)

        self.boostExploration = QtWidgets.QAction(QtGui.QIcon(), 'Boost', mw)
        self.boostExploration.triggered.connect(self.boost)

        self.modeAction = QtWidgets.QAction(QtGui.QIcon(), 'Switch to Play', mw)
        self.modeAction.triggered.connect(self.changemode)

        self.toolbar = mw.addToolBar('Toolbar')
        ...
        self.agent = Agent(self.set_status, self.update_steps, self.update_state
                           , self.update_exploration, self.update_test_steps, self.update_actions)

        self.t = Thread(target=self.agent.train)
        self.t.start()
